    ...
    <van-cell-group>
      <van-cell title="客户姓名" title-class="title">
        <template v-if="!showFooter"
          ><text class="value">{{ data.surname || "" }}</text></template
        >
        <template v-else>
          <van-field
            placeholder="请输入用户名"
      ...
      <van-cell title="手机号码" title-class="title">
        <template v-if="!showFooter"
          ><text class="value">{{ data.mobile || "" }}</text></template
        >
        <template v-else>
          <van-field
            placeholder="请输入手机号码"
      ...
      <van-cell
        title="所属老师"
        title-class="title"
        @click.stop="() => evetnBlurs('staffOptions')"
      >
...
export default {
  ...
  methods: {
    ...
    handlePicker(event) {
      ...
      switch (this.currentPicker) {
        ...
        case "customRage":
          const { values } = deteil;
          const province = values[0].code,
            city = values[1].code,
            district = values[2].code;
  ...
  async created() {
    ...
    const { eduOptions, orgOptions, fromOptions, staffOptions } =
      this.$store.getters;
    this.eduOptions = eduOptions;
    this.fromOptions = fromOptions;
    this.staffOptions = staffOptions.map((item) => ({
      id: item.staff_id,
      name: item.staff_name,
    }));
    this.orgOptions = orgOptions.map((item) => ({
      id: item.institution_id,
      name: item.institution_name,
    }));
    this.customRage = `${this.data.province || ""} ${this.data.city || ""} ${
      this.data.district || ""
    }`;
  },
};
